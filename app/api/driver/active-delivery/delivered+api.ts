...
export async function PATCH(request: Request) {
  ...
  if (!active) {
    return createErrorResponse('No active delivery order to mark as delivered', 400);
  }

  try {
    const order = DriverService.markDelivered(user.driverId, active.id);
    return Response.json({ success: true, message: 'Order marked as delivered', order });
  } catch (err: any) {
